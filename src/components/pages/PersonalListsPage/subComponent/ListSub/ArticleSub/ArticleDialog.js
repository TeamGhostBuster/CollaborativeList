...
export default class ArticleDialog extends React.Component {
  ...
  getArticleInfo() {
    const cb = (response) => {
      ...
      this.setState({ detail: <DetailView title={this.state.title} description={this.state.description} url={this.state.url} tags={this.state.tags} /> });
    };
  ...
  render() {
    ...
    return (
      <Dialog open={this.props.isOpen} actions={actions} autoScrollBodyContent onRequestClose={this.props.close} >
        <Paper>
          <TopBar title={this.state.title} list_id={this.props.list_id} article_id={this.props.id} close={this.props.close} />
          {this.state.detail}
          {this.state.comments}
          <Divider />
